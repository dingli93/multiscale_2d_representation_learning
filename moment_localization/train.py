...
if __name__ == '__main__':
    ...
    if config.upload:
        ...
        # config.resume = config.DATASET.NAME + '_' + 'original'
    ...
    def network(sample):
        ...
        # print(textual_input.shape, textual_mask.shape, visual_input.shape, map_gt.shape, duration)
        # torch.Size([32, 29, 300])
        # torch.Size([32, 29, 1])
        ...
        if not config.TRAIN.multi_scale_2d_map:
            ...
            if config.TRAIN.caption_eval_prop:
                back_caption, target_var, indices, prediction, map_mask = model(textual_input, textual_mask, visual_input)
                r_loss_value, r_loss = getattr(loss, config.RECLOSS.NAME)(back_caption, target_var, textual_input,
                                                                          config.RECLOSS.PARAMS)
                # print(r_loss)
                # print('r_loss', r_loss.shape)
                # print('prediction', prediction.shape)
                # ipdb.set_trace()
                loss_value, joint_prob = getattr(loss, config.RECBCELOSS.NAME)(prediction, map_mask, r_loss, textual_input,
                                                                               indices, config.RECBCELOSS.PARAMS)
                # loss_value, joint_prob = getattr(loss, config.LOSS.NAME)(prediction, map_mask, map_gt, config.LOSS.PARAMS)
                # print('bcapbceloss: ', loss_value)
                # print('reconstruction_loss: ', r_loss_value)
                loss_value += r_loss_value
            else:
            ...
            if config.TRAIN.caption_eval_prop:
                back_caption, target_var, indices, prediction, map_mask = model(textual_input, textual_mask,
                                                                                visual_input)
                # ipdb.set_trace()
                r_loss_value, r_loss = getattr(loss, config.RECLOSS.NAME)(back_caption, target_var, textual_input,
                                                                          config.RECLOSS.PARAMS)
                ...
                # print('prediction', prediction.shape)
                # ipdb.set_trace()
                loss_value, joint_prob = getattr(loss, config.RECBCELOSS.NAME)(prediction, map_mask, r_loss,
                                                                               textual_input,
                                                                               indices, config.RECBCELOSS.PARAMS)
                # loss_value, joint_prob = getattr(loss, config.LOSS.NAME)(prediction, map_mask, map_gt, config.LOSS.PARAMS)
                # print('bcapbceloss: ', loss_value)
                ...
                prediction_list, map_mask_list = model(textual_input, textual_mask, visual_input)
                # ipdb.set_trace()
                loss_value = 0.0
                for i, prediction in enumerate(prediction_list):
                    loss_value_tmp, joint_prob = getattr(loss, config.LOSS.NAME)(prediction, map_mask_list[i], map_gt[i].cuda(),
                                                                             config.LOSS.PARAMS)
                    # print(loss_value)
    ...
    def get_proposal_results(scores, durations):
        ...
        if not config.TRAIN.multi_scale_2d_map:
            ...
            all_scale_out_sorted_times = []
            for j, single_scale_score in enumerate(scores):
                out_sorted_times = []
                for score, duration in zip(single_scale_score, durations):
                    T = score.shape[-1]
                    ...
                    sorted_indexs = torch.from_numpy(sorted_indexs).cuda()
                    # target_size = config.DATASET.NUM_SAMPLE_CLIPS // config.DATASET.TARGET_STRIDE
                    out_sorted_times.append((sorted_indexs.float() / T * duration).tolist())
                # ipdb.set_trace()
                all_scale_out_sorted_times.append(out_sorted_times)

            for i in range(len(out_sorted_times)):
                for j in range(len(all_scale_out_sorted_times) -1):
                    out_sorted_times[i].extend(all_scale_out_sorted_times[j][i])
            # ipdb.set_trace()
            return out_sorted_times
    ...
    def on_update(state):# Save All
        ...
        # if state['t'] % state['test_interval'] == 0 and state['t'] >= 500:
